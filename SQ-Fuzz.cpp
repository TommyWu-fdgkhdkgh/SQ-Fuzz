...
int main(int argc, char *argv[]) {
  ...
  if (optind == argc || !in_dir || !out_dir) usage((u8 *)argv[0]);
  ...
  //make sure "echo core >/proc/sys/kernel/core_pattern"
  //似乎是希望在process crash的時候，資訊是回報給cpu，而不是其他裝置
  check_crash_handling();

  //make sure "echo performance | tee cpu*/cpufreq/scaling_governor"
  //似乎是希望不要使用省電模式，並激發cpu全部的效能
  check_cpu_governor();

  //我猜是“後處理”，執行完某些動作後，自動call
  //  post_handler所指向的function
  setup_post();
  ...
  //對一個陣列進行初始化
  init_count_class8_and_class16();

  //創建工作資料夾（hang, queue....），原本就存在的話
  //  有機率把原本的資料夾刪除
  setup_dirs_fds();
  ...
  //查看執行檔是不是shell，有沒有合法的elf header
  //  並且有沒有插樁
  check_binary(argv[optind]);

  start_time = get_cur_time();

  //假如是qemu mode的話，把現有的argv改成qemu mode的argv
  if (qemu_mode)
  ...
  if (stop_soon) {
    std::cout << "stop fuzzing at 2" << std::endl;
    goto stop_fuzzing;
  }

  /* Woop woop woop */
  if (!not_on_tty) {
    std::cout << "!not_on_tty" << std::endl;
    sleep(4);
    start_time += 4000;
    if (stop_soon) {
      std::cout << "stop fuzzing at 1" << std::endl;
      goto stop_fuzzing;
    }
  }
...
static void save_cmdline(u32 argc, char** argv) {
  ...
  orig_cmdline = (char *)ck_alloc(len);
  buf = orig_cmdline;
...
static void fix_up_banner(char* name) {
...
static void get_core_count(void) {
  ...
  //從/proc/stat這個檔案，就能看到有多少個可用的邏輯cpu了
  ACTF("check how many core we got...");
  ...
  ACTF("check how many runnable process we got...");
...
static double get_runnable_processes(void) {
  ...
  //  /proc/stat
  //  process running : 處於runnable狀態的process個數
  //  process blocked : 處於等待I/O完成的process個數
...
static void maybe_delete_out_dir(void) {
  ...
  if (in_place_resume) {
    ...
    char* orig_q = alloc_printf("%s/queue", out_dir);
  ...
  if (delete_files(fn, (char *)"auto_")) goto dir_cleanup_failed;
  ck_free(fn);
  ...
  if (in_place_resume && rmdir(fn)) {
    ...
    char* nfn = alloc_printf("%s.%04u-%02u-%02u-%02u:%02u:%02u", fn,
                           t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,
                           t->tm_hour, t->tm_min, t->tm_sec);

#else

    char* nfn = alloc_printf("%s_%04u%02u%02u%02u%02u%02u", fn,
                           t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,
                           t->tm_hour, t->tm_min, t->tm_sec);
...
static u8 delete_files(char* path, char* prefix) {
  ...
  while ((d_ent = readdir(d))) {
    ...
    if (d_ent->d_name[0] != '.' && (!prefix ||
      ...
      char* fname = alloc_printf("%s/%s", path, d_ent->d_name);
...
static void read_testcases(void) {
  ...
  for (i = 0; i < nl_cnt; i++) {
    ...
    char* fn = alloc_printf("%s/%s", in_dir, nl[i]->d_name);
    char* dfn = alloc_printf("%s/.state/deterministic_done/%s", in_dir, nl[i]->d_name);
...
static void add_to_queue(char* fname, u32 len, u8 passed_det) {
...
static void load_auto(void) {
  ...
  for (i = 0; i < USE_AUTO_EXTRAS; i++) {
    ...
    char* fn = alloc_printf("%s/.state/auto_extras/auto_%06u", in_dir, i);
    s32 fd, len;
...
static void setup_post(void) {
  ...
  void* dh;
  char* fn = getenv("AFL_POST_LIBRARY");
  u32 tlen = 6;
  ...
  post_handler = (char* (*)(char*, u32*))dlsym(dh, "afl_postprocess");
  if (!post_handler) FATAL("Symbol 'afl_postprocess' not found.");

  /* Do a quick test. It's better to segfault now than later =) */

  post_handler((char *)"hello", &tlen);
...
static void link_or_copy(char* old_path, char* new_path) {
...
static void load_extras_file(char* fname, u32* min_len, u32* max_len,
                             u32 dict_level) {
  ...
  while ((lptr = fgets(buf, MAX_LINE, f))) {
    ...
    *rptr = 0;
    ...
    extras[extras_cnt].data = (u8*)ck_alloc(rptr - lptr);
...
static void load_extras(char* dir) {
  ...
  while ((de = readdir(d))) {
    ...
    struct stat st;
    char* fn = alloc_printf("%s/%s", dir, de->d_name);
    s32 fd;
    ...
    extras[extras_cnt].data = (u8 *)ck_alloc(st.st_size);
...
static void find_timeout(void) {
  ...
  static char tmp[4096]; /* Ought to be enough for anybody. */
...
EXP_ST void check_binary(char* fname) {
  ...
  if (strchr(fname, '/') || !(env_path = getenv("PATH"))) {
    ...
    target_path = (char *)ck_strdup((u8 *)fname);
    ...
    while (env_path) {
      ...
      if (delim) {
        ...
        cur_elem = (char*)ck_alloc(delim - env_path + 1);
        memcpy(cur_elem, env_path, delim - env_path);
        delim++;

      } else cur_elem = (char *)ck_strdup((u8 *)env_path);
  ...
  f_data = (char*)mmap(0, f_len, PROT_READ, MAP_PRIVATE, fd, 0);
...
static char** get_qemu_argv(char* own_loc, char** argv, int argc) {
  ...
  char** new_argv = (char**)ck_alloc(sizeof(char*) * (argc + 4));
  ...
  own_copy = (char *)ck_strdup((u8 *)own_loc);
  ...
  if (!access(BIN_PATH "/afl-qemu-trace", X_OK)) {
    ...
    target_path = new_argv[0] = (char *)ck_strdup((u8 *)BIN_PATH "/afl-qemu-trace");
    return new_argv;
...
static u8 calibrate_case(char** argv, struct queue_entry* q, char* use_mem,
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if (!dumb_mode && !stage_cur && !count_bytes((char *)trace_bits)) {
      fault = FAULT_NOINST;
  ...
  q->bitmap_size = count_bytes((char *)trace_bits);
  q->handicap    = handicap;
  q->cal_failed  = 0;
...
abort_calibration:
  ...
  if (var_detected) {
    ...
    var_byte_count = count_bytes((char *)var_bytes);
...
static void check_map_coverage(void) {
  ...
  if (count_bytes((char *)trace_bits) < 100) return;
...
static void show_stats(void) {
  ...
  sprintf((char *)(tmp + banner_pad), "%s " cLCY VERSION cLGN
...
#define SP20    SP10 SP10
  ...
  if (dumb_mode) {
    ...
    if (queue_cycle == 1 || min_wo_finds < 15) strcpy((char *)tmp, cMGN); else

    /* Subsequent cycles, but we're still making finds. */
    if (cycles_wo_finds < 25 || min_wo_finds < 30) strcpy((char *)tmp, cYEL); else

    /* No finds for a long time and no test cases to try. */
    if (cycles_wo_finds > 100 && !pending_not_fuzzed && min_wo_finds > 120)
      strcpy((char *)tmp, cLGN);

    /* Default: cautiously OK to stop? */
    else strcpy((char *)tmp, cLBL);
  ...
  sprintf((char *)tmp, "%s%s", DI(unique_crashes),
  ...
  sprintf((char *)tmp, "%s%s", DI(unique_hangs),
  ...
  sprintf((char *)tmp, "%s%s (%0.02f%%)", DI(current_entry),
          queue_cur->favored ? "" : "*",
  ...
  sprintf((char *)tmp, "%s (%0.02f%%)", DI(cur_skipped_paths),
          ((double)cur_skipped_paths * 100) / queued_paths);

  SAYF(bV bSTOP " paths timed out : " cRST "%-17s " bSTG bV, tmp);

  sprintf((char *)tmp, "%0.02f bits/tuple",
  ...
  sprintf((char *)tmp, "%s (%0.02f%%)", DI(queued_favored),
  ...
  if (!stage_max) {
    ...
    sprintf((char *)tmp, "%s/-", DI(stage_cur));

  } else {

    sprintf((char *)tmp, "%s/%s (%0.02f%%)", DI(stage_cur), DI(stage_max),
  ...
  sprintf((char *)tmp, "%s (%0.02f%%)", DI(queued_with_cov),
          ((double)queued_with_cov) * 100 / queued_paths);

  SAYF("  new edges on : " cRST "%-22s " bSTG bV "\n", tmp);

  sprintf((char *)tmp, "%s (%s%s unique)", DI(total_crashes), DI(unique_crashes),
          (unique_crashes >= KEEP_UNIQUE_CRASH) ? "+" : "");
  ...
  if (avg_exec < 100) {
    ...
    sprintf((char *)tmp, "%s/sec (%s)", DF(avg_exec), avg_exec < 20 ?
            "zzzz..." : "slow!");
    ...
    sprintf((char *)tmp, "%s/sec", DF(avg_exec));
    SAYF(bV bSTOP "  exec speed : " cRST "%-21s ", tmp);

  }

  sprintf((char *)tmp, "%s (%s%s unique)", DI(total_tmouts), DI(unique_tmouts),
          (unique_hangs >= KEEP_UNIQUE_HANG) ? "+" : "");
  ...
  if (skip_deterministic) {
    ...
    strcpy((char *)tmp, "n/a, n/a, n/a");

  } else {

    sprintf((char *)tmp, "%s/%s, %s/%s, %s/%s",
  ...
  sprintf((char *)tmp, "%s/%s, %s/%s",
          DI(stage_finds[STAGE_HAVOC]), DI(stage_cycles[STAGE_HAVOC]),
          DI(stage_finds[STAGE_SPLICE]), DI(stage_cycles[STAGE_SPLICE]));

  SAYF(bV bSTOP "       havoc : " cRST "%-37s " bSTG bV bSTOP, tmp);

  if (t_bytes) sprintf((char *)tmp, "%0.02f%%", stab_ratio);
    else strcpy((char *)tmp, "n/a");
  ...
  if (!bytes_trim_out) {
    ...
    sprintf((char *)tmp, "n/a, ");

  } else {

    sprintf((char *)tmp, "%0.02f%%/%s, ",
...
static u32 count_bytes(char* mem) {
...
static void mark_as_variable(struct queue_entry* q) {
  ...
  char *fn = strrchr(q->fname, '/') + 1, *ldest;
...
static void save_auto(void) {
  ...
  for (i = 0; i < MIN(USE_AUTO_EXTRAS, a_extras_cnt); i++) {
    ...
    char* fn = alloc_printf("%s/queue/.state/auto_extras/auto_%06u", out_dir, i);
    s32 fd;
...
static void sync_fuzzers(char** argv) {
  ...
  while ((sd_ent = readdir(sd))) {
    ...
    sprintf((char *)stage_tmp, "sync %u", ++sync_cnt);
    stage_name = (char *)stage_tmp;
    stage_cur  = 0;
    stage_max  = 0;
...
static u8 trim_case(char** argv, struct queue_entry* q, u8* in_buf) {
  ...
  while (remove_len >= MAX(len_p2 / TRIM_END_STEPS, TRIM_MIN_BYTES)) {
    ...
    sprintf((char *)tmp, "trim %s/%s", DI(remove_len), DI(remove_len));
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  if (post_handler) {
    ...
    out_buf = (u8 *)post_handler((char *)out_buf, &len);
...
static void write_crash_readme(void) {
  ...
  char* fn = alloc_printf("%s/crashes/README.txt", out_dir);
  s32 fd;
  FILE* f;
...
static u8 fuzz_one(char** argv) {
  ...
  in_buf = (u8 *)mmap(0, len, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
  orig_in = in_buf;
  ...
  if (queue_cur->cal_failed) {
    ...
    if (queue_cur->cal_failed < CAL_CHANCES) {
      ...
      res = calibrate_case(argv, queue_cur, (char *)in_buf, queue_cycle - 1, 0);
...
havoc_stage:
  ...
  if (!splice_cycle) {
    ...
    sprintf((char *)tmp, "splice %u", splice_cycle);
    stage_name  = (char *)tmp;
    stage_short = "splice";
